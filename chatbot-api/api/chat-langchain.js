// LangChain-based Chatbot API with Conversation Memory
// Cost-optimized using GPT-3.5 Turbo + caching
...
// Initialize OpenAI models (using GPT-3.5 for cost efficiency)
...
// Enhanced knowledge base with updated information
const knowledgeBase = [
...
// Initialize vector store (in-memory for simplicity)
...
// System prompt template - first person conversational style
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (conversationHistory.length > 0) {
      // Clear existing memory and rebuild from history
    ...
    const response = result.response || result.output || 'I apologize, but I had trouble processing that. Could you rephrase your question?';
